...
def main() -> int:
    ...
    L.append("Often: covers, vault tracks that mirror their album, or songs that")
    L.append("share heavy lyrical templates (e.g., breakup songs).")
    L.append("")
    ...
    L.append("")
    L.append("Songs whose lyrics are unlike anything else in the corpus.")
    L.append("")
    L.append("| Song | Album | Year | mean sim to top-5 |")
    L.append("|------|-------|------|--------------------|")
    ...
    # ---- cross-album similarity: how often does song X's nearest neighbor come from a different album? ----
    ...
    L.append(f"Of {len(song_records)} songs, **{cross}** ({100*cross/len(song_records):.0f}%) have their nearest")
    L.append("neighbor from a *different* album. High cross-album similarity")
    L.append("suggests Swift draws on a shared lyrical pool across eras —")
    L.append("a song on Folklore can be closest to a song on 1989, etc.")
    L.append("")
